...
public class Spike : MonoBehaviour {
	...
	void OnTriggerEnter2D (Collider2D other) 
	{
		...
		if(other.GetComponent<Actor>())
        {
			...
			if(killable != null && actor)
            {
				OnActorTrigger(actor, killable);
            }
	...
	/// <summary>
	/// This is called for any actor with an IKillable script attached to it
	/// It will kill any IKillable actor if it touches the spikes
	/// </summary>
	/// <param name="actor"></param>
	/// <param name="killable"></param>
    void OnActorTrigger(Actor actor, IKillable killable)
    {
        switch (direction)
        {
            case Directions.Up:
