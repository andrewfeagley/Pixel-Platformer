...
public class WormEnemy : Actor, IKillable, IHittable
{
	...
	/// <summary>
	/// This is called by fsm
	/// fsm acquires all the methods in the script that initializes it and the parses any function with the word _Update in it
	/// It matches the current States enum name to the word that preceds _Update, in this case: Normal
	/// </summary>
    void Normal_Update()
    {
	...
	/// This is called by fsm
	/// fsm acquires all the methods in the script that initializes it and the parses any function with the word _Update in it
	/// It matches the current States enum name to the word that preceds _Update, in this case: Death
	/// </summary>
	void Death_Update () {
		// Horizontal Speed Update Section
		float num = onGround ? 1f : 0.65f;
	...
	/// <summary>
	/// Should be called when the health of the character falls to zero
	/// </summary>
	public void Die () {
		fsm.ChangeState (States.Death, StateTransition.Overwrite);
		//This seems unnecessarily costly
		var kcomponent = GetComponent<Knockable>();
		if (kcomponent)
        {
			Destroy(kcomponent);
        }
	...
	/// <summary>
	/// This should be called by an event on Knockable
	/// It should have an amount to calculate how long the actor should stay in hitstun
	/// </summary>
	/// <param name="a"></param>
	/// <param name="b"></param>
