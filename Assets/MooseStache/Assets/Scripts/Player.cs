...
public class Player : Actor, IKillable
{
	...
	//These have been moved to the ActorData Scriptable Object class
	/*
	...
	*/
	...
	new void Update () {
		...
		if (onGround)
		{
			wallSlideTimer = actorData.WallSlideTime; // Reset the wall slide timer when on the ground
			jumpGraceTimer = actorData.JumpGraceTime;
	...
	void Normal_Update () {
		...
		if (((moveX > 0 && CheckColInDir (Vector2.right * -1f, solid_layer)) || (moveX < 0 && CheckColInDir (Vector2.right, solid_layer))) && canStick && !onGround) {
			stickTimer = actorData.clingTime;
			sticking = true;
			canStick = false;
		}
		...
		*/
		...
		if (!onGround) {
			...
			if ((moveX == (int)Facing) && moveY != -1) {
				...
				if (wallSlideDir != 0) {
					target = Mathf.Lerp (actorData.MaxFall, actorData.WallSlideStartMax, wallSlideTimer / actorData.WallSlideTime);
				}
			}
	...
	private void Ducking_Update () {
		...
		Speed.x = Calc.Approach (Speed.x, 0f, actorData.RunReduce * num * Time.deltaTime);
	...
	private void LadderClimb_Update () {
		...
		Speed.y = Calc.Approach (Speed.y, actorData.LadderClimbSpeed * moveY, 10000 * Time.deltaTime);
	...
	private IEnumerator Dash_Enter () {
		dashCooldownTimer = actorData.DashCooldown;
		Speed = Vector2.zero;
		...
		value.Normalize();
		Vector2 vector = value * actorData.DashSpeed;
		Speed = vector;
		DashDir = value;
		if (DashDir.x != 0f) {
		...
		yield return new WaitForSeconds (actorData.DashTime);

		// Wait one extra frame
		yield return null;
			
		if (DashDir.y >= 0f) {
			Speed = DashDir * actorData.EndDashSpeed;
		}
		if (Speed.y > 0f) {
			Speed.y = Speed.y * actorData.EndDashUpMult;
	...
	// Jump Function
	public void Jump () {
		wallSlideTimer = actorData.WallSlideTime;
		jumpGraceTimer = 0f;
		jumpBufferTimer = 0f;
		varJumpTimer = actorData.VariableJumpTime;
		Speed.x = Speed.x + actorData.JumpHBoost * (float)moveX;
		Speed.y = actorData.JumpSpeed;
	...
	private void WallJump(int dir) {
		...
		if (moveX != 0) {
			forceMoveX = dir;
			forceMoveXTimer = actorData.WallJumpForceTime;
		}

		Speed.x = actorData.WallJumpHSpeed * (float)dir;
		Speed.y = actorData.JumpSpeed;
		varJumpSpeed = Speed.y;
		SpriteScale = new Vector2(0.6f, 1.4f);
	...
	private bool WallJumpCheck(int dir) {
		...
		if (dir == -1) {
			leftcorner = new Vector2 (myCollider.bounds.center.x - myCollider.bounds.extents.x - .5f - (float)actorData.WallJumpCheckDist, myCollider.bounds.center.y + myCollider.bounds.extents.y - 1f);
			...
			rightcorner = new Vector2 (myCollider.bounds.center.x + myCollider.bounds.extents.x + .5f + (float)actorData.WallJumpCheckDist, myCollider.bounds.center.y - myCollider.bounds.extents.y + 1f);
		}
	...
	public void SpringBounce() {
		varJumpTimer = actorData.SpringJumpVariableTime; // Set the amount of time people can hold the jump button to reach higher
		wallSlideTimer = actorData.WallSlideTime; // reset the wall slide timer
		...
		Speed.y = actorData.SpringJumpSpeed; // Set the Y speed to the desired one 
